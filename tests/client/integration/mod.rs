...
fn client_starts_in_welcome_mode() {
    ...
    let (_, client) = Client::spawn(input.clone(), output.clone(), TtyIoCtlConfig { rows: 5, cols: 10 });
    let mut vterm = build_vterm(ScreenSize { rows: 5, cols: 10 });
...
fn client_can_enter_command_mode() {
    ...
    let (tx, client) = Client::spawn(input.clone(), output.clone(), TtyIoCtlConfig { rows: 5, cols: 10 });
...
fn client_can_enter_program_mode() {
    ...
    let (tx, client) = Client::spawn(input.clone(), output.clone(), TtyIoCtlConfig { rows: 24, cols: 80 });
